export default function reducerFunc(prev, action) {
  switch (action.type) {
    case "LOGIN":
      const currentUser = prev.users.filter(
        (el) => el.userName === action.name
      )[0];
    ...
    case "SIGNUP":
      const newUser = {
        ...
        online: true,
        userID:
          prev.users.length < 10
            ? `0${prev.users.length + action.name}`
            : `${prev.users.length + action.name}`,
      };
      return { ...prev, users: [...prev.users, newUser], currentUser: newUser };

    case "USERCHANNELS":
      const userChannels = prev.channels.filter((el) =>
        el.members.some((id) => id === action.id)
      );
    ...
    case "LEAVE_CHANNEL":
      const leaveChannel = prev.channels.map((el, i) => {
        if (el.channelName === action.name) {
          const newMembers = el.members.filter(
            (id) => id !== prev.currentUser.userID
          );
          return { ...el, members: newMembers };
        } else return el;
      });

      const leaveCurrentUserChannels = prev.currentUserChannels.filter(
        (el, i) => el.channelName !== action.name
      );
    ...
    case "JOIN_CHANNEL":
      ...
      const joinCurrentUserChannels = [
        ...prev.currentUserChannels,
        joinChannel[toAddToCurrentUserChannels],
      ];
    ...
    case "ADD_USER":
      ...
      const newChannel_addPeople = prev.channels.map((el) => {
        if (el.channelName === action.channelName)
          return { ...el, members: [...el.members, user.userID] };
        else return el;
      });
      const newCurrentUserChannels_addPeople = prev.currentUserChannels.map(
        (el) => {
          if (el.channelName === action.channelName)
            return { ...el, members: [...el.members, user.userID] };
          else return el;
      ...
      return {
        ...prev,
        channels: newChannel_addPeople,
        currentUserChannels: newCurrentUserChannels_addPeople,
      };

    case "UPDATE":
      if (prev.currentUser) {
        const newcurrentUserChannels1 = action.channels.filter((el) =>
          el.members.some((id) => id === prev.currentUser.userID)
        );
        return {
          ...prev,
          users: [...action.users],
          channels: [...action.channels],
          currentUserChannels: newcurrentUserChannels1,
        };
      } else {
        return {
          ...prev,
